...
//        NSLog(@"%@",_conversation.chatter);
//        NSMutableDictionary *paramDic = [NSMutableDictionary dictionaryWithCapacity:1];
//        [paramDic setValue:_conversation.chatter forKey:@"groupId"];
...
//
//        }];
//        [EaseMessageModel sendGetOrPostRequest:@"listener/info"param:paramDic requestStyle:HTTPRequestTypePost setSerializer:HTTPResponseTypeJSON handleBlock:^(id data, NSError *error, FMIErrorModelTwo *errorModel)
//         {
//             [self hideHud];
//             if (data)
//             {
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:NSLocalizedString(@"录音时间太短", @"录音时间太短")];//media.timeShort,The recording time is too short
...
{
//    message.ext[@"uid"] = message.from;
//    [FKXUserManager addExtMessageWithEMMessage:message];
//    [self insertDataToTableWith:message managedObjectContext:ApplicationDelegate.managedObjectContext];
